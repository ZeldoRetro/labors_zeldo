...
function map:on_opening_transition_finished()
  ...
  sol.timer.start(map,16500,function()
    ...
    if i < 31 then return true end
  end)

  sol.timer.start(map,16500 * 12,function()
    hero:teleport("cutscenes/end_screen_wave_2")
  end)
end

function map:on_key_pressed(key)
  if key == "space" then
    if not game:get_value("game_finished_2") then return end
    sol.audio.play_music(nil) hero:teleport("cutscenes/end_screen_wave_2")
  end
end
