...
function chest_game_manager:create_chest_game(map, prefix, price, rewards)
  ...
  local rules_dialog
  if game:get_map():get_world() == "inside_world" then rules_dialog = "chest_game.rules_light" else rules_dialog = "chest_game.rules" end
  ...
  function chest_npc:on_interaction()
    ...
    if playing then
      ...
      if minigame_done then
        -- Propose to replay.
        game:start_dialog("chest_game.replay", function(answer)
          if answer == 1 then
            -- Yes.
            if game:get_money() < price then
              ...
              game:start_dialog("chest_game.thanks", function()
                ...
                map:set_entities_enabled("wall",false)
        ...
        -- Propose to play.
        game:start_dialog(rules_dialog, function(answer)
          if answer == 1 then
            -- Yes.
            if game:get_money() < price then
              game:start_dialog("chest_game.not_enough_money")
  ...
  local function on_opened(chest)
    ...
    if not playing then
      ...
      local max_chest
      if game:get_map():get_world() == "inside_world" then max_chest = 0 else max_chest = 2 end
