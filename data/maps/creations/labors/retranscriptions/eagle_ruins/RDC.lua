...
map:register_event("on_started",function(map, destination)
  ...
  if destination == entree_donjon then
    ...
    -- Arc ?
    if game:get_value("get_bow_10021") then
    ...
    -- Boomerang ?
    if game:get_value("get_boomerang_10021") then
...
-- PORTE A CLÉ 1 OUVERTE EN SORTANT DE LA MAP
map:register_event("on_finished",function(map)
  game:set_value("key_10021_1_opened", true)
end)

-- EXCEPTION : Activer sensor porte fermée enlève les sensor_push_hero de chaque coté
sensor_falling_auto_door_2:register_event("on_activated", function(sensor)
  map:set_entities_enabled("sensor_push_hero_auto_door_2", false)
end)