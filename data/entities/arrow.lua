...
-- Replaces the built-in one to allow other types of arrows.
...
arrow:set_can_traverse("stairs", true)
...
arrow:add_collision_test("sprite", function(arrow, entity)
  ...
  if entity:get_type() == "enemy" then
    ...
    if enemy:get_arrow_reaction() == "ignored" then return end
...
arrow:add_collision_test("overlapping", function(arrow, entity)
  ...
  elseif entity_type == "switch" then
    ...
    if flying and sprite ~= nil and
         (sprite:get_animation_set() == "entities/Switches/solid_switch" or
         sprite:get_animation_set() == "entities/Switches/switch_invisible_target" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_blue_down" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_blue_left" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_blue_right" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_blue_up") then
    ...
    -- Dark eyes (for light arrows)
    if flying and sprite ~= nil and game:get_value("get_bow_light") and
         (sprite:get_animation_set() == "entities/Switches/switch_eye_dark_down" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_dark_left" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_dark_right" or
         sprite:get_animation_set() == "entities/Switches/switch_eye_dark_up") then
      if not switch:is_activated() then
        sol.audio.play_sound("switch")
        switch:set_activated(true)
        if switch:on_activated() ~= nil then switch:on_activated() end
...
function arrow:go()
  ...
  movement:set_max_distance(5000)
