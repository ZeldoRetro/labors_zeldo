...
function item:on_using()
  ...
  local function set_can_traverse_rules(entity)
    ...
    entity:set_can_traverse_ground("low_wall", false)
  ...
  function go()
    ...
    local distance
    if item:get_variant() == 2 then distance = 192 else distance = 160 end      --FORGOTTEN LEGEND : 144 THEN 192
    local speed
    if item:get_variant() == 2 then speed = 225 else speed = 196 end
  ...
  hookshot:add_collision_test("overlapping", function(hookshot, entity)
    ...
    elseif entity_type == "switch" then
      ...
      if not hooked and not going_back and sprite:get_animation_set() == "entities/Switches/solid_switch" then
