--Bomb bag: Allows you to have bombs and use them.

local item = ...

function item:on_created()
  self:set_savegame_variable("possession_bomb_bag")
...
function item:on_variant_changed(variant)
  ...
  local bombs_counter = self:get_game():get_item("inventory/bombs_counter")
  local bomb = self:get_game():get_item("pickable/bomb")
  if variant == 0 then
    bombs_counter:set_max_amount(0)
    bomb:set_obtainable(false)
  else
    local max_amounts = {20, 40, 99}
