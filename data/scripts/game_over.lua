...
function game_over_menu:on_started()
  ...
  fairy_sprite = sol.sprite.create("menus/arrow")
  fairy_sprite:set_animation("blinking")
  state = "waiting_start"
  fairy_npc = sol.sprite.create("entities/items")
  fairy_npc:set_animation("pickable/fairy")
    ...
    fade_sprite.on_animation_finished = function()
      ...
      elseif state == "opening_menu" then
        ...
        if bottle_with_fairy ~= nil then
          ...
          fairy_npc:set_xy(hero_dead_x + 12, hero_dead_y + 21)
          local movement = sol.movement.create("target")
          movement:set_target(24, 22)
          movement:set_speed(96)

          movement:start(fairy_npc, function()
            state = "waiting_end"
            game:add_life(8 * 4)  -- Restore 8 hearts.
            sol.audio.play_sound("fairy")
            sol.timer.start(self, 1000, function()
              state = "resume_game"
              hero:set_invincible()
              hero:set_blinking()
              sol.timer.start(game, 1000, function() hero:set_invincible(false) hero:set_blinking(false) end)
