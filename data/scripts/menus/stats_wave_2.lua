...
local function create_menu_title_widget(game)
  local widget = gui_designer:create(160, 28)
  widget:set_xy(16, 8)
  widget:make_text(sol.language.get_string("stats_menu.title_wave_2"), 6, 6, "left")
  return widget
end
...
function stats_manager:new(game)
  ...
  local function get_items_inventory_string()
    ...
    if game:get_value("labors_casualization_wave_2") then num_items_inventory = num_items_inventory + 1 end
    if game:get_value("labors_quiver_wave_2") then num_items_inventory = num_items_inventory + 1 end
    if game:get_value("labors_bomb_bag_wave_2") then num_items_inventory = num_items_inventory + 1 end
    if game:get_value("labors_magic_flask_upgrade_wave_2") then num_items_inventory = num_items_inventory + 1 end
    if game:get_value("labors_defense_boost_wave_2") then num_items_inventory = num_items_inventory + 1 end
    if game:get_value("labors_attack_boost_wave_2") then num_items_inventory = num_items_inventory + 1 end
  ...
  local function get_items_dungeons_string()
    ...
    if game:get_value("red_key_10010_1") then num_items_dungeons = num_items_dungeons + 1 end
    if game:get_value("blue_key_10010_1") then num_items_dungeons = num_items_dungeons + 1 end
    if game:get_value("green_key_10010_1") then num_items_dungeons = num_items_dungeons + 1 end
    if game:get_value("yellow_key_10010_1") then num_items_dungeons = num_items_dungeons + 1 end
    if game:get_value("purple_key_10010_1") then num_items_dungeons = num_items_dungeons + 1 end
  ...
  local function get_items_main_quest_string()
    ...
    if game:get_value("get_trophy_10011") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10012") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10013") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10014") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10015") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10016") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10017") then num_items_main_quest = num_items_main_quest + 1 end
    if game:get_value("get_trophy_10018") then num_items_main_quest = num_items_main_quest + 1 end
    max_items_main_quest = 8
    return tr("stats_menu.items_main_quest") .. " " .. num_items_main_quest .. " / "  ..max_items_main_quest
  end

  local function get_force_gems_string()

    num_force_gems = game:get_value("remembrance_shard_1st_solarus_quest_found")
    max_force_gems = 150
  ...
  function stats:on_command_pressed(command)
    ...
    if command == "action" then
      ...
      game:set_value("game_finished_2",true)
  ...
  function stats:on_started()
    if percent == 100 then
      menu_background_picture = sol.surface.create("menus/stats_picture_platinum_1st_solarus_quest.png")
      game:set_value("door_platinum_rank_10010_1_opened",true)
    elseif percent <= 99 and percent >= 90 then
      menu_background_picture = sol.surface.create("menus/stats_picture_gold_1st_solarus_quest.png")
    elseif percent <= 89 and percent >= 60 then
      menu_background_picture = sol.surface.create("menus/stats_picture_silver_1st_solarus_quest.png")
    else
      menu_background_picture = sol.surface.create("menus/stats_picture_bronze_1st_solarus_quest.png")
    end
