--Inventory Menu: Assignables items and equipment items
...
--  item_names[10] = game:has_item("inventory/magic_powder") and "inventory/magic_powder" or "inventory/magic_mushroom"
...
function inventory_manager:new(game)
  ...
  local function set_cursor_position(row, column)
    ...
    if variant > 0 then
      item_caption = sol.language.get_string("inventory.item_caption." .. item_name .. "." .. variant)
  ...
  function inventory:on_command_pressed(command)
    ...
    elseif command == "item_1" then
      ...
      if item == game:get_item("inventory/ocarina") and game:has_item("inventory/ocarina") then 
        sol.audio.play_sound("assign_item") 
        game:switch_pause_menu("songs")
	  elseif item == game:get_item("treasures/treasures_bag") and game:has_item("treasures/treasures_bag") then 
        sol.audio.play_sound("assign_item") 
        game:switch_pause_menu("treasures")
      elseif item == game:get_item("treasures/spoils_bag") and game:has_item("treasures/spoils_bag") then 
        sol.audio.play_sound("assign_item") 
        game:switch_pause_menu("loots")
      elseif item == game:get_item_assigned(2) then
        sol.audio.play_sound("assign_item")
    ...
    elseif command == "item_2" then
      ...
      elseif item == game:get_item("treasures/spoils_bag") and game:has_item("treasures/spoils_bag") then 
        sol.audio.play_sound("assign_item") 
        game:switch_pause_menu("loots")
      elseif item == game:get_item_assigned(1) then
        sol.audio.play_sound("assign_item")
